...
class GraphvizFlowVisualizer:
    ...
    def create_artifact_flow_diagram(self, output_format: str = 'svg') -> graphviz.Digraph:
        ...
        for artifact_name, artifact in self.artifacts.items():
            ...
            if artifact.description:
                desc = artifact.description[:30] + "..." if len(artifact.description) > 30 else artifact.description
                label += f"\\n{desc}"
                
            dot.node(f"artifact_{artifact_name}", label, 
                    shape='diamond', style='filled', fillcolor='#F0F8FF')
                    
        # Add phase nodes
        for phase_id, phase in self.phases.items():
            dot.node(f"phase_{phase_id}", phase.name, 
                    shape='box', style='rounded,filled', fillcolor='#E6E6FA')
        ...
        for artifact_name, artifact in self.artifacts.items():
            ...
            for producer in artifact.producers:
                if producer in self.phases:
                    dot.edge(f"phase_{producer}", artifact_node, 
                            label='produces', color='green')
                            
            # Artifact -> Consumers
            for consumer in artifact.consumers:
                if consumer in self.phases:
                    dot.edge(artifact_node, f"phase_{consumer}", 
    ...
    def create_combined_diagram(self, output_format: str = 'svg') -> graphviz.Digraph:
        ...
        with dot.subgraph(name='cluster_phases') as phases_cluster:
            phases_cluster.attr(label='Execution Phases', style='rounded', bgcolor='#F5F5F5')
            ...
            for phase_id, phase in self.phases.items():
                color = status_colors.get(phase.status, '#E6E6FA')
                label = f"{phase.name}\\n({phase.status})"
                phases_cluster.node(phase_id, label, 
        ...
        with dot.subgraph(name='cluster_artifacts') as artifacts_cluster:
            artifacts_cluster.attr(label='Data Artifacts', style='rounded', bgcolor='#F0F8FF')
            
            for artifact_name, artifact in self.artifacts.items():
                label = artifact_name
                artifacts_cluster.node(f"artifact_{artifact_name}", label,
                                     shape='diamond', style='filled', fillcolor='#87CEEB')
    ...
    def generate_diagrams(self, output_dir: Path, formats: List[str] = ['svg', 'png']):
        ...
        for fmt in formats:
            artifact_diagram.render(output_dir / f'artifact_flow', format=fmt, cleanup=True)
            
        # Generate combined diagram
        combined_diagram = self.create_combined_diagram()
        for fmt in formats:
            combined_diagram.render(output_dir / f'combined_flow', format=fmt, cleanup=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate control flow diagrams with Graphviz")
    ...
    parser.add_argument("--formats", nargs='+', default=['svg', 'png'], 
                       help="Output formats")
    parser.add_argument("--type", choices=['phases', 'artifacts', 'combined'], 
                       default='combined', help="Diagram type")
    parser.add_argument("--show-source", action='store_true', 
                       help="Show Graphviz DOT source")
    parser.add_argument("--compare", action='store_true',
                       help="Show comparison with Mermaid")
