...
class OrchestratorRegenerator:
    ...
    def regenerate_global_orchestrator(
        self, orchestrator_file: Path, flow_name: str = "main_config_flow"
    ) -> bool:
        """
        ...
        if "# Infrastructure imports" not in new_content:
            ...
            if step_imports_end in new_content:
                ...
                new_content = new_content.replace(
                    step_imports_end, f"{step_imports_end}\n\n{infrastructure_template}"
                )
    ...
    def _detect_existing_units(
        self, library_dir: Path, units: List[Dict[str, Any]]
    ) -> List[str]:
        """
    ...
    def _update_library_init(
        ...
        new_content = self._replace_between_markers(
            original_content, "LIBRARY_IMPORTS", imports_section
        )

        # Replace MOCK_UNITS section
        new_content = self._replace_between_markers(
            new_content, "MOCK_UNITS", mocks_section
        )

        return new_content

    def _replace_between_markers(
        self, content: str, marker_name: str, new_content: str
    ) -> str:
        """
    ...
    def _generate_library_imports(
        self, units: List[Dict[str, Any]], existing_units: List[str]
    ) -> str:
        ...
        for unit in units:
            ...
            if unit_id in existing_units:
                ...
                module_name = file_name.replace(".py", "")
                lines.append(
                    f"# from .{module_name} import {class_name}  # TODO: Implement real unit"
                )
    ...
    def _generate_library_init(
        self, library_name: str, units: List[Dict[str, Any]], existing_units: List[str]
    ) -> str:
        """
        ...
        for unit in units:
            ...
            if unit_id in existing_units:
                ...
                lines.append(
                    f"    'Mock{class_name}',  # Using mock until real unit is implemented"
                )
        lines.append(f"]")
        lines.append(f"")
    ...
    def _replace_section(self, content: str, section_name: str, new_code: str) -> str:
        """
    ...
    def _generate_phase_imports(self, phases: List[Dict[str, Any]]) -> str:
        ...
        for phase in sorted(phases, key=lambda p: p.get("sequence", 0)):
            ...
            orchestrator_module = (
                f".phase_{sequence}_{phase_id}.orchestrator_{phase_id}"
            )
    ...
    def _generate_phase_sequence(self, phases: List[Dict[str, Any]]) -> str:
        ...
        for phase in sorted(phases, key=lambda p: p.get("sequence", 0)):
            ...
            lines.append(
                f"    {{'phase_id': '{phase_id}', 'sequence': {sequence}, 'status': '{status}'}},"
            )
    ...
    def _generate_static_execution(self, phases: List[Dict[str, Any]]) -> str:
        ...
        for i, phase in enumerate(
            sorted(phases, key=lambda p: p.get("sequence", 0)), 1
        ):
            phase_id = phase["phase_id"]
            ...
            lines.append(
                f"# Phase {i}: {phase_name} (sequence={phase.get('sequence', i)})"
            )
    ...
    def _generate_step_imports(
        self, steps: List[Dict[str, Any]], phase: Dict[str, Any]
    ) -> str:
...
'''
    ...
    def _prepare_phase_template_data(
        self, phase: Dict[str, Any], flow: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        class_name = (
            "".join(word.capitalize() for word in phase_id.split("_")) + "Phase"
        )
        ...
        for step in phase.get("steps", []):
            ...
            step_class_name = (
                "".join(word.capitalize() for word in step_id.split("_")) + "Step"
            )
            steps.append(
                {
                    "step_id": step_id,
        ...
        return {
            "phase": {
                ...
                "phase_directory": phase.get("implementation", {}).get(
                    "phase_directory", f"phases/phase_{phase_id}"
                ),
    ...
    def _generate_dual_mode_orchestrator(
        self, orchestrator_file: Path, phase_data: Dict[str, Any]
    ) -> bool:
        """
    ...
    def _update_dual_mode_orchestrator(
        ...
        try:
            ...
            if not has_execute_dynamic:
                ...
                lines = self._insert_method_before_execute(
                    lines, dynamic_method, "_execute_dynamic"
                )
    ...
    def _add_mode_parameter(self, lines: List[str]) -> List[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if in_init and not added and "ui=" in line:
                ...
                new_lines.append(
                    " " * indent + "phase_spec: Optional[Dict[str, Any]] = None"
                )
                added = True
                continue
...
'''
    ...
    def _insert_method_before_execute(
        self, lines: List[str], method_code: str, method_name: str
    ) -> List[str]:
    ...
    def _wrap_execute_with_mode_switch(self, lines: List[str]) -> List[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if in_execute and not first_statement_found:
                ...
                new_lines.append(
                    " " * (execute_indent + 4) + 'if self.mode == "dynamic":'
                )
                new_lines.append(
                    " " * (execute_indent + 8) + "return self._execute_dynamic(context)"
                )
...
def main():
    ...
    parser.add_argument(
        "--spec", type=Path, required=True, help="Path to control_flows.yml"
    )
    parser.add_argument(
        "--orchestrator",
        type=Path,
        help="Path to orchestrator file (not required for library type)",
    )
    parser.add_argument(
        "--type",
        choices=["global", "phase", "library"],
        required=True,
    ...
    parser.add_argument(
        "--library-name", help="Library name (required for library type)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Output directory for library (default: phases/libraries/)",
    )
    parser.add_argument(
        "--dual-mode",
        action="store_true",
        help="Generate/update with dual-mode support (hardcoded + dynamic)",
    )
    ...
    if args.type == "library":
        ...
        success = regenerator.regenerate_global_orchestrator(
            orchestrator_file=args.orchestrator, flow_name=args.flow
        )
    else:  # phase
        if not args.orchestrator:
