...
class TransformationHistory:
    ...
    def _load_history(self) -> Dict[str, Any]:
        ...
        if self.history_file.exists():
            try:
                ...
                print(f"⚠️  Warning: Could not parse {self.history_file}, creating new history")
    ...
    def record_transformation(
        ...
        entry = {
            ...
            "validation_valid": plan.validation_result.valid if plan.validation_result else None,
            "validation_errors": plan.validation_result.errors if plan.validation_result else [],
            "validation_warnings": plan.validation_result.warnings if plan.validation_result else [],
    ...
    def can_rollback(self, steps: int = 1) -> Tuple[bool, str]:
        ...
        if len(transformations) < steps:
            return False, f"Not enough transformations in history ({len(transformations)} < {steps})"
    ...
    def record_rollback(
        ...
        entry = {
            ...
            "flow_name": "multiple" if steps > 1 else rolled_back[0].get("flow_name", "unknown"),
        ...
        if spec_file and spec_file.exists():
            entry["checksums_after"][str(spec_file)] = self._calculate_checksum(spec_file)
...
class DirectorySynchronizer:
    ...
    def _plan_step_operations(
        ...
        if mapping.operation == 'delete':
            ...
            old_step_dir = self._get_step_directory(phase_seq, phase_name, mapping.old_sequence, step_name)
            ...
            old_step_dir = self._get_step_directory(phase_seq, phase_name, mapping.old_sequence, step_name)
            new_step_dir = self._get_step_directory(phase_seq, phase_name, mapping.new_sequence, step_name)
    ...
    def execute_operations(self, operations: Optional[List[DirectoryOperation]] = None) -> Dict[str, Any]:
        """
    ...
    def preview_operations(self, operations: Optional[List[DirectoryOperation]] = None) -> str:
...
class CodePathUpdater:
        ...
        for op in dir_operations:
            if op.operation == 'move' and op.source and op.destination:
                ...
                try:
                    ...
                    self.path_mappings[str(old_rel).replace('\\', '/')] = str(new_rel).replace('\\', '/')
    ...
    def find_config_files(self) -> List[Path]:
        ...
        for pattern in ['**/*.yml', '**/*.yaml', '**/*.json', '**/*.toml', '**/*.ini', '**/*.cfg']:
            ...
            try:
                ...
                for old_path, new_path in self.path_mappings.items():
                    ...
                    if re.search(pattern1, updated_content):
                        ...
                        changes_made.append(f"from {old_path} import → from {new_path} import")
                ...
                if updated_content != content:
                    ...
                    files_updated += 1
                    print(f"  ✓ Updated {py_file.relative_to(self.base_path)}: {len(changes_made)} import(s)")
        ...
        for config_file in config_files:
            try:
                ...
                if updated_content != content:
                    ...
                    files_updated += 1
                    print(f"  ✓ Updated {config_file.relative_to(self.base_path)}: {len(set(changes_made))} path(s)")
        ...
        for doc_file in doc_files:
            try:
                ...
                for old_path, new_path in self.path_mappings.items():
                    ...
                    patterns = [
                        # [text](path)
                        (rf'\[([^\]]+)\]\({old_escaped}([^\)]*)\)', lambda m: f'[{m.group(1)}]({new_path}{m.group(2)})'),
                        # `path`
                        (rf'`{old_escaped}`', f'`{new_path}`'),
                ...
                if updated_content != content:
                    ...
                    files_updated += 1
                    print(f"  ✓ Updated {doc_file.relative_to(self.base_path)}: {len(set(changes_made))} path(s)")
...
class ControlFlowTransformation:
    ...
    def plan_renumber(
        ...
        plan = TransformationPlan(
            ...
            flow_name=flow_name,
            description=f"Renumber sequences in {flow_name}" + (f" (phase: {phase_id})" if phase_id else ""),
            metadata={
        ...
        if 'phases' in flow and phase_id is None:
            ...
            for idx, phase in enumerate(sorted_phases):
                ...
                new_seq = start_from + idx if strategy == "compact" else max(old_seq, start_from + idx)
                
                if old_seq != new_seq:
                    plan.add_mapping(TransformationMapping(
                        element_type='phase',
        ...
        if 'phases' in flow:
            ...
            if phase_id:
                phases_to_process = [p for p in phases_to_process if p.get('phase_id') == phase_id]
            ...
            for phase in phases_to_process:
                if 'steps' in phase:
                    ...
                    for step_idx, step in enumerate(sorted_steps):
                        ...
                        new_step_seq = start_from + step_idx if strategy == "compact" else max(old_step_seq, start_from + step_idx)
    ...
    def plan_insert(
        ...
        plan = TransformationPlan(
            ...
            flow_name=flow_name,
            description=f"Insert {element_type} '{new_id}'" + (f" in phase {phase_id}" if phase_id else ""),
            metadata={
                'phase_id': phase_id,
    ...
    def plan_delete(
        ...
        plan = TransformationPlan(
            ...
            flow_name=flow_name,
            description=f"Delete {element_type} '{element_id}'" + (f" from phase {phase_id}" if phase_id else ""),
            metadata={
            ...
            for phase in flow['phases']:
                if 'steps' in phase:
                    ...
                    if len(step_seqs) != len(set(step_seqs)):
                        result.add_error(f"Duplicate step sequences in phase '{phase.get('phase_id')}'")
    ...
    def apply(
        ...
        if plan.validation_result is None:
            raise ValueError("Plan must be validated before applying. Call validate() first.")
        ...
        if sync_directories:
            if not project_base_path:
                ...
                if self.spec_file:
                    ...
                    raise ValueError("project_base_path required when sync_directories=True")
            
            print(f"\n🔄 Synchronizing directory structure...")
            synchronizer = DirectorySynchronizer(project_base_path, dry_run=dry_run_sync)
            ...
            if dir_operations:
                ...
                if update_code_paths and dir_operations:
                    print(f"\n📝 Updating code paths...")
                    path_updater = CodePathUpdater(project_base_path, dry_run=dry_run_sync)
                    ...
                    print(f"    Total files updated: {path_results['total_files_updated']}")
            else:
        ...
        if not dry_run_sync:  # Only record actual transformations, not dry runs
            ...
            if update_code_paths and 'updates' in locals():
                ...
                affected_files = [Path(u['file']) for u in path_results.get('updates', [])]
        ...
        if regenerate_orchestrators and not dry_run_sync:
            if not project_base_path:
                if self.spec_file:
                    ...
                    print("⚠️  Cannot regenerate orchestrators: project_base_path not provided")
                    return new_spec
    ...
    def get_persistent_history(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def _create_inverse_transformation(self, entry: Dict[str, Any]) -> TransformationPlan:
        """
        ...
        if trans_type == "renumber":
            ...
            raise ValueError(f"Cannot create inverse for transformation type: {trans_type}")
    ...
    def rollback(
        ...
        for i, entry in enumerate(to_rollback, 1):
            ...
            try:
                ...
                current_spec = self._apply_plan_to_virtual_spec(inverse_plan, current_spec)
...
def scaffold_transformer(
    ...
    """
    from .scaffolder import ScaffoldGenerator, PhaseInsertion, StepInsertion, ImplementationStatus
    ...
    if not spec_exists:
        ...
        if step_data:
            ...
            if create_directories and project_base_path:
                ...
                phase_id = step_data.get('phase_id') or transformer.original_spec['phase']['phase_id']
                phase_seq = transformer.original_spec['phase']['sequence']
                phase_dir = Path(project_base_path) / "phases" / f"phase_{phase_seq}_{phase_id}"
