...
from .scaffolder import ScaffoldGenerator, StepInsertion, PhaseInsertion
...
class ControlFlowDesigner:
    ...
    @classmethod
    def from_existing(
        cls, spec_file: Path, project_root: Optional[Path] = None
    ) -> "ControlFlowDesigner":
        """
    ...
    def add_phase(
        ...
        phase_data = {
            ...
            "implementation": {
                ...
                "class": phase.orchestrator_class_name
                or f'{phase.phase_id.title().replace("_", "")}Phase',
        ...
        print(
            f"✅ Added phase '{phase.phase_id}' to flow '{flow_name}' at position {insert_index}"
        )
        ...
        if create_scaffolding and phase.create_scaffolding:
            created_files = self.scaffolder.create_phase_scaffolding(
                phase=phase, base_path=self.project_root / "phases"
            )
    ...
    def add_step(
        ...
        if create_scaffolding and step.create_scaffolding:
            phase_dir = (
                self.project_root / "phases" / f"phase_{step.phase_sequence}_{phase_id}"
            )
            created_files = self.scaffolder.create_step_scaffolding(
                step=step, base_path=phase_dir
            )
        ...
        if update_orchestrator:
            ...
            if orchestrator_file.exists():
                ...
                print(
                    f"   Create phase scaffolding first or manually add step to orchestrator"
                )
    ...
    def update_phase_status(
        ...
        return self.manager.update_phase(
            flow_name=flow_name, phase_id=phase_id, updates={"status": status.value}
        )
        ...
        for flow_name, flow in self.manager.flows.items():
            ...
            for phase in flow["phases"]:
                ...
                if phase_id in phase_ids:
                    errors.append(
                        f"Duplicate phase_id '{phase_id}' in flow '{flow_name}'"
                    )
                ...
                if "steps" in phase:
                    ...
                    for step in phase["steps"]:
                        ...
                        if step_id in step_ids:
                            errors.append(
                                f"Duplicate step_id '{step_id}' in phase '{phase_id}'"
                            )
            ...
            if sequences:
                if sorted(sequences) != list(range(min(sequences), max(sequences) + 1)):
                    warnings.append(
                        f"Flow '{flow_name}' has non-contiguous sequence numbers"
                    )
    ...
    def get_implementation_progress(
        self, flow_name: str = "main_config_flow"
    ) -> Dict[str, Any]:
        """
        ...
        overall_percentage = (
            (implemented_steps / total_steps * 100) if total_steps > 0 else 0
        )
    ...
    def delete_phase(
        self, phase_id: str, cascade_renumber: bool = True, preview_only: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def move_phase(
        self, from_sequence: int, to_sequence: int, preview_only: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def swap_phases(
        self, sequence_a: int, sequence_b: int, preview_only: bool = False
    ) -> Dict[str, Any]:
        """
        Swap two phases.
        ...
        try:
            ...
            if not phase_a_id or not phase_b_id:
                return {"success": False, "message": "One or both phases not found"}
    ...
    def swap_steps(
        ...
        try:
            ...
            if not target_phase:
                return {"success": False, "message": f"Phase '{phase_id}' not found"}
            ...
            if not step_a_id or not step_b_id:
                return {"success": False, "message": "One or both steps not found"}
    ...
    def reorder_phases(
        self, new_order: Dict[int, int], preview_only: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            operation = ReorderOperation(
                new_order=new_order, element_path="phases", id_field="phase_id"
            )
    ...
    def reorder_steps(
        self, phase_id: str, new_order: Dict[int, int], preview_only: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def rollback_transformation(
        self, steps: int = 1, preview_only: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def preview_transformation(self, operation: str, **kwargs) -> Dict[str, Any]:
        """
...
def main():
    ...
    print(
        f"Phases: {progress['phases']['implemented']}/{progress['phases']['total']} "
        f"({progress['phases']['percentage']:.1f}%)"
    )
    print(
        f"Steps: {progress['steps']['implemented']}/{progress['steps']['total']} "
