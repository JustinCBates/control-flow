...
def visualize(spec_file, output, format):
    ...
    try:
        ...
        from control_flow_engine.visualizer.mermaid_generator import (
...
def analyze(spec_file, output):
    ...
    try:
        ...
        artifacts = content.count("artifacts_produced:") + content.count(
            "artifacts_consumed:"
        )
        ...
        if output:
            ...
            click.echo(
                f"  Complexity Score: {report['summary']['complexity_score']:.1f}"
            )
