...
class ControlFlowEditor:
    ...
    def __init__(self, project_root: str):
        ...
        self.designer = ControlFlowDesigner(
            spec_file=self.spec_file, project_root=self.project_root
        )
            ...
            if not phases_dict:
                ...
                if flows:
                    ...
                    flow_name = (
                        ...
                        if "main_deployment_flow" in flows
                        else list(flows.keys())[0]
                    )
    ...
    def _renumber_menu(self):
        ...
        if scope == "phase_steps":
            ...
            phase_id = questionary.select(
                "Select phase:", choices=phase_choices, style=custom_style
            ).ask()
        ...
        if questionary.confirm(
            "Apply this renumbering?", default=False, style=custom_style
        ).ask():
            print("\n⏳ Applying changes...")
            result = self.designer.renumber_phase(
                phase_id=phase_id,
    ...
    def _insert_phase(self):
        ...
        name = questionary.text(
            "Phase Name:", validate=lambda x: len(x) > 0 or "Name cannot be empty"
        ).ask()
        ...
        if phases:
            ...
            for phase in sorted_phases:
                ...
                position_choices.append(
                    {"name": f"After '{phase_name}'", "value": f"after:{pid}"}
                )
                position_choices.append(
                    {"name": f"Before '{phase_name}'", "value": f"before:{pid}"}
                )

        position = questionary.select(
            "Position:", choices=position_choices, style=custom_style
        ).ask()
        ...
        cascade = questionary.confirm(
            "Cascade renumber subsequent phases?", default=True, style=custom_style
        ).ask()

        # Build phase data
        phase_data = {"phase_id": phase_id, "name": name}
        if description:
        ...
        # Apply?
        if questionary.confirm(
            "Insert this phase?", default=False, style=custom_style
        ).ask():
            print("\n⏳ Inserting phase...")
            result = self.designer.insert_phase(
                phase_data=phase_data,
    ...
    def _insert_step(self):
        ...
        name = questionary.text(
            "Step Name:", validate=lambda x: len(x) > 0 or "Name cannot be empty"
        ).ask()
        ...
        if steps:
            ...
            for step in sorted_steps:
                ...
                position_choices.append(
                    {"name": f"After '{step_name}'", "value": f"after:{sid}"}
                )
                position_choices.append(
                    {"name": f"Before '{step_name}'", "value": f"before:{sid}"}
                )
        ...
        # Cascade renumber?
        cascade = questionary.confirm(
            "Cascade renumber subsequent steps?", default=True, style=custom_style
        ).ask()

        # Build step data
        step_data = {"step_id": step_id, "name": name}
        if description:
        ...
        # Apply?
        if questionary.confirm(
            "Insert this step?", default=False, style=custom_style
        ).ask():
            print("\n⏳ Inserting step...")
            result = self.designer.insert_step(
                phase_id=phase_id,
    ...
    def _move_phase(self):
        ...
        preview_result = self.designer.move_phase(
            from_sequence=from_seq, to_sequence=to_seq, preview_only=True
        )
        ...
        confirm = questionary.confirm(
            "\nApply these changes?", default=False, style=custom_style
        ).ask()

        if confirm:
            result = self.designer.move_phase(
                from_sequence=from_seq, to_sequence=to_seq, preview_only=False
            )
    ...
    def _swap_phases(self):
        ...
        preview_result = self.designer.swap_phases(
            sequence_a=seq_a, sequence_b=seq_b, preview_only=True
        )
        ...
        # Confirm
        confirm = questionary.confirm(
            "\nApply this swap?", default=False, style=custom_style
        ).ask()

        if confirm:
            result = self.designer.swap_phases(
                sequence_a=seq_a, sequence_b=seq_b, preview_only=False
            )
    ...
    def _swap_steps(self):
        ...
        preview_result = self.designer.swap_steps(
            phase_id=phase_id, sequence_a=seq_a, sequence_b=seq_b, preview_only=True
        )
    ...
    def _reorder_phases(self):
        ...
        new_order_input = questionary.text(
            "\nEnter new order:",
            validate=lambda x: all(s.strip().isdigit() for s in x.split(","))
            or "Must be comma-separated numbers",
        ).ask()
        ...
        preview_result = self.designer.reorder_phases(
            new_order=new_order_map, preview_only=True
        )
        ...
        if confirm:
            result = self.designer.reorder_phases(
                new_order=new_order_map, preview_only=False
            )
    ...
    def _reorder_steps(self):
        ...
        preview_result = self.designer.reorder_steps(
            phase_id=phase_id, new_order=new_order_map, preview_only=True
        )
        ...
        if confirm:
            result = self.designer.reorder_steps(
                phase_id=phase_id, new_order=new_order_map, preview_only=False
            )
    ...
    def _delete_phase(self):
        ...
        phase_id = questionary.select(
            "Select phase to delete:", choices=phase_choices, style=custom_style
        ).ask()

        # Cascade renumber?
        cascade = questionary.confirm(
            "Cascade renumber remaining phases?", default=True, style=custom_style
        ).ask()

        # Preview
        print("\n⏳ Generating preview...")
        result = self.designer.delete_phase(
            phase_id=phase_id, cascade_renumber=cascade, preview_only=True
        )
        ...
        if questionary.confirm(
            ...
            print("\n⏳ Deleting phase...")
            result = self.designer.delete_phase(
                phase_id=phase_id, cascade_renumber=cascade, preview_only=False
            )
    ...
    def _delete_step(self):
        ...
        step_id = questionary.select(
            "Select step to delete:", choices=step_choices, style=custom_style
        ).ask()

        # Cascade renumber?
        cascade = questionary.confirm(
            "Cascade renumber remaining steps?", default=True, style=custom_style
        ).ask()
    ...
    def _rollback_menu(self):
        ...
        steps = questionary.text(
            ...
            validate=lambda x: x.isdigit()
            and int(x) > 0
            and int(x) <= len(history)
            or f"Must be 1-{len(history)}",
        ).ask()
        ...
        result = self.designer.rollback_transformation(steps=steps, preview_only=True)
        ...
        # Confirm rollback
        if questionary.confirm(
            f"⚠️  Rollback {steps} transformation(s)?", default=False, style=custom_style
        ).ask():
            print("\n⏳ Rolling back...")
            result = self.designer.rollback_transformation(
                steps=steps, preview_only=False
            )
