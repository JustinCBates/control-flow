        ...
        for rule in self.rules:
            ...
            if rule.check_type == ValidationType.REQUIRED_FIELD:
                if not exists:
                    msg = (
                        rule.error_message
                        or f"Required field missing: '{rule.field_path}'"
                    )
...
def demo_validator():
    ...
    # Valid data
    valid_data = {"settings": {"timeout": 30, "enabled": True}}
    ...
    invalid_data = {
        "settings": {"timeout": "30", "enabled": True}  # String instead of int
    }
    ...
    # Valid nested data
    valid_data = {"config": {"database": {"host": "localhost", "port": 5432}}}
