...
class DeleteResult:
    ...
    def __repr__(self):
        if self.success:
            affected = len([m for m in self.mappings if m.old_sequence != m.new_sequence or m.is_deleted])
            ...
            for parent in parent_list:
                if parent.get(parent_id_field) == operation.parent_id:
                    parent[operation.element_path] = [
                        item for item in parent.get(operation.element_path, [])
                        if item.get(operation.id_field) != operation.element_id
                    ]
        else:
            # Top-level deletion
            modified[operation.element_path] = [
                item for item in modified.get(operation.element_path, [])
                if item.get(operation.id_field) != operation.element_id
            ]
...
def demo_deleter():
    ...
    operation = DeleteOperation(
        element_id="item2",
        cascade_renumber=False,  # No cascade
        element_path="items"
    )
