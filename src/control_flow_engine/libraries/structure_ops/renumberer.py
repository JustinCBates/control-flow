...
class RenumberResult:
    ...
    def __repr__(self):
        if self.success:
            affected = len([m for m in self.mappings if m.old_sequence != m.new_sequence])
    ...
    def renumber(
        ...
        try:
            ...
            if not target_list:
                return RenumberResult(
                    success=True,
                    errors=["Target list is empty - nothing to renumber"]
                )
...
def demo_renumberer():
    ...
    operation = RenumberOperation(
        strategy=RenumberStrategy.SHIFT_DOWN,
        deletion_point=2,
        element_path="items"
    )
