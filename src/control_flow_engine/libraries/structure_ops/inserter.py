...
class InsertOperation:
    ...
    element_path: str = (
        "items"  # Path to list in structure (e.g., "items", "phases", "steps")
    )
        ...
        if self.success:
            affected = len(
                [
                    ...
                    if m.old_sequence != m.new_sequence or m.is_new_element
                ]
            )
...
class StructureInserter:
    ...
    def insert_element(
        ...
        try:
            ...
            mappings = self._calculate_mappings(target_list, insert_seq, operation)

            # Apply changes if not dry run
            modified_structure = structure
            if not dry_run:
                modified_structure = self._apply_insertion(
                    structure, operation, insert_seq, mappings
                )
    ...
    def _validate_operation(self, operation: InsertOperation):
        ...
        if operation.id_field not in operation.new_element:
            raise InsertError(
                f"New element missing required field '{operation.id_field}'"
            )
        ...
        elif operation.insertion_point.position in [
            InsertPosition.BEFORE,
            InsertPosition.AFTER,
        ]:
            if not operation.insertion_point.reference_id:
                raise InsertError(
                    f"{operation.insertion_point.position.value} position requires reference_id"
                )

    def _get_target_list(
        self, structure: Dict[str, Any], operation: InsertOperation
    ...
    def _calculate_insert_sequence(
        self, target_list: List[Dict[str, Any]], operation: InsertOperation
    ) -> Optional[int]:
        ...
        if operation.insertion_point.position == InsertPosition.AT_END:
            ...
            for item in target_list:
                if (
                    item.get(operation.id_field)
                    == operation.insertion_point.reference_id
                ):
                    return item.get(seq_field, 0) + 1
            raise InsertError(
                f"Reference element '{operation.insertion_point.reference_id}' not found"
            )
    ...
    def _apply_insertion(
        ...
        for mapping in mappings:
            if (
                not mapping.is_new_element
                and mapping.old_sequence != mapping.new_sequence
            ):
                for item in target_list:
