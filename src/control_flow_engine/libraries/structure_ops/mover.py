...
class MoveOperation:
    ...
    def is_reparent(self) -> bool:
        ...
        return self.new_parent_path is not None and self.parent_path != self.new_parent_path
    ...
    def summary(self) -> str:
        ...
        if self.is_reparent():
            lines.append(f"  Parent change: {self.parent_path} → {self.new_parent_path}")
...
class StructureMover:
    ...
    def move_element(
        ...
        if element_index is None:
            raise MoveError(f"No element found with sequence {from_sequence} at path {element_path}")
        ...
        try:
            if not dry_run:
                self._apply_move(parent_list, element_index, from_sequence, to_sequence, direction, affected_mappings)
        ...
        for key in path:
            if isinstance(current, dict):
                ...
                try:
                    ...
                    raise MoveError(f"Invalid path: cannot access index '{key}' in list")
            else:
    ...
    def _find_element_by_sequence(self, elements: List[Dict], sequence: int) -> Optional[int]:
    ...
    def _validate_move(self, parent_list: List[Dict], operation: MoveOperation) -> List[str]:
        ...
        if operation.to_sequence > max_sequence:
            errors.append(f"Target sequence {operation.to_sequence} exceeds list size {max_sequence}")
