...
class SwapResult:
    ...
    def __repr__(self):
        if self.success and self.operation:
            return (f"✅ SUCCESS: Swapped '{self.operation.element_a_id}' (seq {self.operation.element_a_sequence}) "
...
class StructureSwapper:
    ...
    def swap_elements(
        ...
        if index_a is None:
            return SwapResult(
                success=False,
                errors=[f"No element found with sequence {sequence_a}"]
            )
        
        if index_b is None:
            return SwapResult(
                success=False,
                errors=[f"No element found with sequence {sequence_b}"]
            )
        ...
        if validate:
            validation_errors = self._validate_swap(parent_list, operation, index_a, index_b)
        ...
        for key in path:
            if isinstance(current, dict):
                ...
                try:
                    ...
                    raise SwapError(f"Invalid path: cannot access index '{key}' in list")
            else:
