...
class TerminalCapabilities:
    ...
    def _detect_capabilities(self):
        ...
        # VS Code integrated terminal detection
        self.is_vscode = (
            'vscode' in self.term_program.lower() or
            self.vscode_injection != '' or
        ...
        self.is_limited = (
            self.term_type in limited_terms or
            not self.is_tty or
            self.is_vscode  # VS Code terminal has arrow key issues with questionary
        )
        
        # Arrow key support
        self.supports_arrow_keys = (
            self.has_questionary and
            not self.is_limited and
            self.is_tty
        )
...
class UniversalMenu:
    ...
    def _select_questionary(
        ...
        result = self.questionary.select(
            message,
            choices=choices,
            style=style
        ).ask()
        ...
        while True:
            try:
                ...
                if number in choice_map:
                    ...
                    print(f"⚠️  Invalid choice. Please enter a number from {min(valid_numbers)} to {max(valid_numbers)}")
            except ValueError:
    ...
    def _confirm_questionary(
        ...
        result = self.questionary.confirm(
            message,
            default=default,
            style=style
        ).ask()
