...
class PhaseStepRenumberer:
    ...
    def renumber_steps(
        ...
        if len(phase_dirs) > 1:
            raise ValidationError(
                f"Multiple phase {phase_number} directories found: {phase_dirs}"
            )
    ...
    def compact_phase_numbering(self, dry_run: bool = True) -> RenumberPlan:
        """
    ...
    def _scan_current_numbering(
        self, scope: str, parent_dir: Optional[Path] = None  # 'phases' or 'steps'
    ) -> List[int]:
        """
    ...
    def _create_compact_plan(
        self, current_numbers: List[int], scope: str, parent_dir: Optional[Path] = None
    ) -> RenumberPlan:
    ...
    def _get_paths_for_number(
        self, old_num: int, new_num: int, scope: str, parent_dir: Optional[Path]
        ...
        for py_file in py_files:
            try:
                ...
                for old_name, new_name in replacements.items():
                    ...
                    patterns = [
                        (
                            rf"\bfrom\s+phases\.{re.escape(old_name)}",
                            f"from phases.{new_name}",
                        ),
