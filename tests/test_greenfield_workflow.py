...
class TestGreenfieldWorkflow:
    ...
    def test_complete_greenfield_workflow(self, temp_project):
        ...
        assert (
            phase_dir / "orchestrator_initialization.py"
        ).exists(), "Orchestrator should exist"
        ...
        assert (
            step_dir / "load_config.py"
        ).exists()  # Implementation file is named {step_id}.py
        ...
        assert (
            "from .step_0_load_config.load_config import LoadConfigStep"
        ...
        assert (
            "from .step_1_validate_config.validate_config import ValidateConfigStep"
            in orchestrator_content
        )
        assert "Step 1: Load configuration from file" in orchestrator_content
    ...
    def test_add_multiple_phases(self, temp_project):
        ...
        designer = ControlFlowDesigner.new_project(
            project_name="Multi-Phase Project", project_root=temp_project
        )
    ...
    def test_validation_catches_errors(self, temp_project):
        ...
        designer = ControlFlowDesigner.new_project(
            project_name="Validation Test", project_root=temp_project
        )
        ...
        print(
            f"✅ Validation caught {len(report.errors)} errors and {len(report.warnings)} warnings"
        )
