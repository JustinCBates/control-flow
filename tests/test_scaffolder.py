...
class TestScaffoldGenerator:
    ...
    def test_create_phase_scaffolding():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = generator.create_phase_scaffolding(
                phase=phase, base_path=project / "phases", force=False
            )
            ...
            assert (
                phase_dir / "orchestrator_test.py"
            ).exists(), "Orchestrator not created"
    ...
    def test_create_step_scaffolding():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = generator.create_step_scaffolding(
                step=step, base_path=phase_dir, force=False
            )
            ...
            assert (
                step_dir / "test_step.py"
            ).exists(), "Implementation file not created"
    ...
    def test_phase_overwrite_with_force():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Recreate with force
            result2 = generator.create_phase_scaffolding(
                phase=phase, base_path=project / "phases", force=True
            )
    ...
    def test_generated_content():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check init file
            init_content = (
                project / "phases" / "phase_2_analysis" / "__init__.py"
            ).read_text()
            assert "Phase 2 - Data Analysis" in init_content, "Init missing phase info"
            assert (
                "DataAnalysisOrchestrator" in init_content
            ), "Init missing orchestrator import"

            # Check orchestrator file
            orch_content = (
                project / "phases" / "phase_2_analysis" / "orchestrator_analysis.py"
            ).read_text()
            assert (
                "DataAnalysisOrchestrator" in orch_content
            ), "Orchestrator missing class name"
            assert (
                "Analyze the data thoroughly" in orch_content
            ), "Orchestrator missing description"

            # Check README
            readme_content = (
                project / "phases" / "phase_2_analysis" / "README.md"
            ).read_text()
            assert "# Phase 2: Data Analysis" in readme_content, "README missing title"
            assert (
                "Analyze the data thoroughly" in readme_content
