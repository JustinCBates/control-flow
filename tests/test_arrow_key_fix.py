...
def main():
    ...
    confirmed = menu.confirm(
        "Does the menu work correctly in VS Code terminal?", default=True
    )
    ...
    if confirmed:
        ...
        print(
            f"   - Terminal: {menu.capabilities.term_type if menu.capabilities else 'unknown'}"
        )
    ...
    feedback = menu.text("Any feedback on the fix? (optional)", default="Works great!")
...
def confirm_fix(menu: UniversalMenu):
    ...
    q1 = menu.confirm("Did you see a menu (either arrows or numbers)?", default=True)

    if not q1:
        print("⚠️  Menu didn't appear. Please check terminal setup.")
        return

    q2 = menu.confirm("Were you able to select an option?", default=True)

    if not q2:
        print("⚠️  Selection didn't work. Please report this issue.")
        return

    q3 = menu.confirm("Did it work without arrow keys (if in VS Code)?", default=True)
