...
def test_example_file():
    ...
    example_files = ["designer_transformation_demo.py"]
    ...
    for filename in example_files:
        ...
        print_test(
            f"Example '{filename}' exists", exists, str(filepath) if not exists else ""
        )
        if not exists:
            all_passed = False
...
def test_ui_files():
    ...
    ui_files = ["flow_editor.py", "README.md"]
    ...
    for filename in ui_files:
        ...
        exists = filepath.exists()
        print_test(
            f"UI file '{filename}' exists", exists, str(filepath) if not exists else ""
        )
        if not exists:
            all_passed = False
