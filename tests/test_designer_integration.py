...
class TestHarness:
    ...
    def test_api_renumber(self):
        ...
        self.assert_true(sequences == [1, 2, 3], f"Phases renumbered correctly: {sequences}")
        ...
        phases = spec['flows']['test_flow']['phases']
        sequences = [p['sequence'] for p in phases]
        self.assert_true(sequences == [0, 1, 2], f"Phases renumbered from 0: {sequences}")
        
        # Test 3: Renumber steps in specific phase
        result = self.designer.renumber_phase(phase_id='init', start_from=1, strategy='compact')
        self.assert_success(result, "Renumber steps in 'init' phase")
        
        spec = self.manager.get_specification()
        init_phase = next(p for p in spec['flows']['test_flow']['phases'] if p['phase_id'] == 'init')
        step_sequences = [s['sequence'] for s in init_phase['steps']]
        self.assert_true(step_sequences == [1, 2], f"Steps renumbered: {step_sequences}")
    ...
    def test_api_insert(self):
        ...
        result = self.designer.insert_phase(phase_data=phase_data, cascade_renumber=True)
        ...
        self.assert_true(validate_idx == init_idx + 1, f"Phase inserted after init: {phase_ids}")
    ...
    def test_api_delete(self):
        ...
        phases = spec['flows']['test_flow']['phases']
        phase_ids = [p['phase_id'] for p in phases]
        self.assert_true('validate_phase' not in phase_ids, f"Phase deleted: {phase_ids}")
    ...
    def test_preview_mode(self):
        ...
        self.assert_true('preview_test' not in phase_ids, "Phase not inserted (preview only)")
    ...
    def test_edge_cases(self):
        ...
        result = self.designer.insert_step(
            phase_id='nonexistent_phase',
            step_data=step_data
        )
        self.assert_true(not result['success'], "Insert into non-existent phase rejected")
    ...
    def test_real_world_scenarios(self):
        ...
        for step_id, step_name in [('check_input', 'Check Input'), ('verify', 'Verify')]:
            result = self.designer.insert_step(
                phase_id='validation',
                step_data={'step_id': step_id, 'name': step_name}
            )
        ...
        validation_phase = next((p for p in phases if p['phase_id'] == 'validation'), None)
        if validation_phase:
            self.assert_true(
