...
class MockScanner:
    ...
    def __init__(self, project_root: Path, control_flows_file: Path):
        ...
        with open(control_flows_file, 'r') as f:
    ...
    def find_mock_files(self, phase_id: str = None) -> List[Path]:
        ...
        if phase_id:
            ...
            for phase in phases:
                if phase.get('id') == phase_id:
                    ...
                    phase_dir = self.project_root / "phases" / f"phase_{phase_sequence}_{phase_id}"
                    break
        ...
        for search_dir in search_dirs:
            for pattern in self.MOCK_PATTERNS:
                for mock_file in search_dir.rglob(pattern.split('/')[-1] if '**/' in pattern else pattern):
                    # Check exclusions
    ...
    def check_mock_referenced(self, mock_file: Path) -> Tuple[bool, List[Tuple[Path, int]]]:
        """
    ...
    def remove_obsolete_mocks(self, scan_results: Dict, dry_run: bool = True) -> int:
        ...
        for mock_info in obsolete_mocks:
            ...
            print(f"{'[DRY RUN] ' if dry_run else ''}Removing: {mock_file.relative_to(self.project_root)}")
        ...
        print(f"{'='*70}")
        print(f"{'[DRY RUN] Would remove' if dry_run else 'Removed'}: {removed_count} file(s)")
...
def main():
    ...
    parser.add_argument(
        "--phase",
        help="Limit scan to specific phase ID"
    )
    parser.add_argument(
        "--auto-remove",
        action='store_true',
    ...
    if args.control_flows:
        ...
        if not control_flows_file.exists():
            control_flows_file = project_root.parent / "control-flow" / "control_flows.yml"
                ...
                'obsolete_mocks': [
                    {
                        ...
                        'size': m['size'],
                        'reason': m['reason'],
                        'implementation_files': [str(f) for f in m['implementation_files']]
                    }
