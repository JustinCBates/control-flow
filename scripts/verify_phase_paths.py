...
class PhasePathValidator:
    ...
    def __init__(self, project_root: Path, verbose: bool = False):
        ...
        self.control_flows_file = self.project_root / "design_specs" / "control_flows.yml"
        self.errors = []
        self.warnings = []
        self.info = []
        
        if not self.control_flows_file.exists():
            raise FileNotFoundError(f"control_flows.yml not found at {self.control_flows_file}")
    ...
    def validate_directory_sequence(self, physical_phases: List[Tuple[int, str, Path]]) -> bool:
        """
        ...
        if sequences != expected:
            self.log_error(f"Directory sequences not consecutive: found {sequences}, expected {expected}")
    ...
    def validate_yaml_consistency(self, spec: Dict, physical_phases: List[Tuple[int, str, Path]]) -> bool:
        """
        ...
        if len(yaml_phases) != len(physical_phases):
            self.log_error(f"Phase count mismatch: YAML has {len(yaml_phases)}, directories have {len(physical_phases)}")
            all_valid = False
        
        # Build lookup dict
        physical_dict = {phase_id: (seq, path) for seq, phase_id, path in physical_phases}
        ...
        for yaml_phase in yaml_phases:
            ...
            if phase_id not in physical_dict:
                self.log_error(f"Phase '{phase_id}' in YAML but no matching directory found")
                all_valid = False
                continue
            ...
            if yaml_seq != physical_seq:
                self.log_error(f"Phase '{phase_id}': YAML sequence={yaml_seq}, directory sequence={physical_seq}")
                all_valid = False
            
            # Check phase_directory path
            expected_dir = f"phases/phase_{physical_seq}_{phase_id}/"
            if yaml_dir != expected_dir:
                self.log_warning(f"Phase '{phase_id}': YAML phase_directory='{yaml_dir}', expected '{expected_dir}'")
                all_valid = False
            
            # Check orchestrator_file path
            yaml_orch = yaml_phase.get('implementation', {}).get('orchestrator_file', '')
            expected_orch = f"phases/phase_{physical_seq}_{phase_id}/orchestrator_{phase_id}.py"
            if yaml_orch != expected_orch:
                self.log_warning(f"Phase '{phase_id}': YAML orchestrator_file='{yaml_orch}', expected '{expected_orch}'")
    ...
    def validate_code_constants(self, physical_phases: List[Tuple[int, str, Path]]) -> bool:
        """
        ...
        for seq, phase_id, phase_path in physical_phases:
            ...
            if not orchestrator_file:
                self.log_warning(f"Phase '{phase_id}': No orchestrator file found (checked: {orchestrator_patterns})")
                continue
            ...
            if code_seq is None:
                self.log_warning(f"Phase '{phase_id}': No PHASE_SEQUENCE constant found in {orchestrator_file.name}")
                continue
            
            # Compare
            if code_seq != seq:
                self.log_error(f"Phase '{phase_id}': Code PHASE_SEQUENCE={code_seq}, directory sequence={seq}")
                all_valid = False
            else:
                self.log_info(f"Phase '{phase_id}': PHASE_SEQUENCE constant matches ({seq})")
    ...
    def fix_yaml_paths(self, spec: Dict, physical_phases: List[Tuple[int, str, Path]]) -> bool:
        """
        ...
        for yaml_phase in yaml_phases:
            ...
            if yaml_phase.get('sequence') != physical_seq:
                print(f"  Updating {phase_id}: sequence {yaml_phase.get('sequence')} → {physical_seq}")
                yaml_phase['sequence'] = physical_seq
                changes_made = True
            
            # Update phase_directory
            expected_dir = f"phases/phase_{physical_seq}_{phase_id}/"
            if yaml_phase.get('implementation', {}).get('phase_directory') != expected_dir:
                print(f"  Updating {phase_id}: phase_directory → {expected_dir}")
                yaml_phase.setdefault('implementation', {})['phase_directory'] = expected_dir
                changes_made = True
            
            # Update orchestrator_file
            expected_orch = f"phases/phase_{physical_seq}_{phase_id}/orchestrator_{phase_id}.py"
            if yaml_phase.get('implementation', {}).get('orchestrator_file') != expected_orch:
                print(f"  Updating {phase_id}: orchestrator_file → {expected_orch}")
                yaml_phase.setdefault('implementation', {})['orchestrator_file'] = expected_orch
                changes_made = True
    ...
    def run_validation(self, fix_yaml: bool = False) -> bool:
        ...
        if all_valid:
            ...
            print(f"   control_flows.yml: {'✅ Consistent' if yaml_valid else '❌ Inconsistent'}")
            print(f"   Code constants: {'✅ Correct' if code_valid else '❌ Incorrect'}")
...
Examples:
  ...
  python3 verify_phase_paths.py --project-root /path/to/project
    ...
    parser.add_argument('--fix-yaml', action='store_true', help='Update control_flows.yml to match directories')
