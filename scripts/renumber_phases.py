...
class PhaseRenumberer:
    ...
    def update_phase_sequence_in_file(self, file_path: Path, old_seq: int, new_seq: int) -> bool:
        ...
        for match in matches:
            if int(match.group(2)) == old_seq:
                if self.dry_run:
                    print(f"    [DRY RUN] Would update {file_path.name}: sequence {old_seq} → {new_seq}")
                    return True
                
                new_content = re.sub(pattern, rf"\g<1>{new_seq}\g<3>", content)
                file_path.write_text(new_content)
                print(f"    ✅ Updated {file_path.name}: sequence {old_seq} → {new_seq}")
                return True
        
        return False
    
    def rename_phase_directory(self, old_num: int, new_num: int, phase_id: str) -> Optional[Path]:
    ...
    def update_orchestrator_file(self, phase_dir: Path, phase_id: str, old_seq: int, new_seq: int):
    ...
    def insert_phase(self, position: int, phase_name: str, phase_id: str):
        ...
        if position < 1 or position > len(phases) + 1:
            print(f"❌ Error: Position {position} is out of range (1-{len(phases) + 1})")
            return False
        ...
        for i in range(len(phases) - 1, position - 1, -1):
            ...
            old_dir_path = phase.get('implementation', {}).get('phase_directory', f"phases/phase_{old_seq}_{phase_id_current}/")
            new_dir_path = f"phases/phase_{new_seq}_{phase_id_current}/"
            phase['implementation']['phase_directory'] = new_dir_path
            phase['implementation']['orchestrator_file'] = f"{new_dir_path}orchestrator_{phase_id_current}.py"
            ...
            if new_dir and not self.dry_run:
                self.update_orchestrator_file(new_dir, phase_id_current, old_seq, new_seq)
        ...
        if not self.dry_run:
            ...
            print(f"     python3 phases/phase_{position}_{phase_id}/orchestrator_{phase_id}.py --show-paths")
    ...
    def delete_phase(self, position: int):
        ...
        for i in range(position - 1, len(phases)):
            ...
            phase['sequence'] = new_seq
            phase['implementation']['phase_directory'] = f"phases/phase_{new_seq}_{phase_id_current}/"
            phase['implementation']['orchestrator_file'] = f"phases/phase_{new_seq}_{phase_id_current}/orchestrator_{phase_id_current}.py"
    ...
    def move_phase(self, from_pos: int, to_pos: int):
        ...
        if from_pos < 1 or from_pos > len(phases):
            print(f"❌ Error: Source position {from_pos} is out of range (1-{len(phases)})")
            return False
        
        if to_pos < 1 or to_pos > len(phases):
            print(f"❌ Error: Target position {to_pos} is out of range (1-{len(phases)})")
            return False
        ...
        if from_pos < to_pos:
            ...
            for pos in range(from_pos + 1, to_pos + 1):
                ...
                phase['implementation']['phase_directory'] = f"phases/phase_{new_pos}_{phase_id_current}/"
                phase['implementation']['orchestrator_file'] = f"phases/phase_{new_pos}_{phase_id_current}/orchestrator_{phase_id_current}.py"
                
                if new_dir and not self.dry_run:
                    self.update_orchestrator_file(new_dir, phase_id_current, pos, new_pos)
        else:
        ...
        moved_phase['implementation']['phase_directory'] = f"phases/phase_{to_pos}_{phase_id}/"
        moved_phase['implementation']['orchestrator_file'] = f"phases/phase_{to_pos}_{phase_id}/orchestrator_{phase_id}.py"
...
Examples:
  ...
  python3 renumber_phases.py insert --at 3 --name "Test" --id test --dry-run
    ...
    insert_parser.add_argument('--id', dest='phase_id', required=True, help='Phase identifier (lowercase_with_underscores)')
    
    # Delete command
    delete_parser = subparsers.add_parser('delete', help='Delete a phase')
    delete_parser.add_argument('--at', type=int, required=True, help='Position to delete (1-based)')
    
    # Move command
    move_parser = subparsers.add_parser('move', help='Move a phase')
    move_parser.add_argument('--from', dest='from_pos', type=int, required=True, help='Current position (1-based)')
    move_parser.add_argument('--to', dest='to_pos', type=int, required=True, help='New position (1-based)')
